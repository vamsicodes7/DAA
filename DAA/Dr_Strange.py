...
def shortest_path(universes, m, portals, demon_times):
    # Create a list to store the shortest travel time to each universe
    shortest_time = [float('inf')] * (universes + 1)
    # Initialize the starting universe with time 0
    shortest_time[1] = 0

    # Create a priority queue to store the universes to be visited
    ...
    while pq:
        ...
        # Iterate through the portals connected to the current universe
        for portal in portals[current_universe]:
            next_universe, travel_time = portal

            # Calculate the next arrival time considering demon patrolling
...
for i in range(1, n + 1):
    demon_times[i] = list(map(int, input().split()))[1:]  # Skip the first integer (ki)
